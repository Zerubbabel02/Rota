...
// Talks to the same Supabase edge function the web/React layer already uses
// (tap-transfer-claim) plus the auth token refresh endpoint — all via plain
// HttpURLConnection so tapping Accept on a notification (app closed, no
// WebView/JS running at all) can still complete a claim.
object RotaApiClient {
    ...
    fun claim(context: Context, token: String): ClaimResult {
        ...
        if (accessToken == null) {
            Log.w(TAG, "claim: no stored access token — was storeSession ever called for this account?")
